...
const GRID_SIZE = 5;
const IMAGE_SRC = "/assets/level7.jpg";
...
function Gameplay1() {
  ...
  const handleDrop = (index) => {
    ...
    if (isSolved(newPieces)) {
      ...
      setTimeout(() => {
        MySwal.fire({
          title: "Bravo!",
          html: `Completion Time: <strong>${formatTime(timer)}</strong>!<br/>New Gallery: <strong>Level 2</strong><br/><br/>`
  ...
  const saveScoreToFirebase = async (time) => {
    ...
    if (snapshot.exists()) {
      ...
      const updatedTime = (data.total_time || 0) + time;
      const updatedLevels = (data.levels_completed || 0) + 1;
